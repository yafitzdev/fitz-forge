...
class SynthesisStage(PipelineStage):
    ...
    @staticmethod
    def _extract_schema_fields(prior_outputs: dict[str, Any]) -> dict[str, list[str]]:
        """AST-extract Pydantic model fields from schema/model source files.

        Returns {ClassName: ["fieldname: type", ...]} for annotated fields
        on classes found in files with 'schema' or 'models/' in their path.
        Used by _ground_artifact_fields to detect wrong field name usage.
        """
        import ast as _ast
        ...
        _SCHEMA_KEYWORDS = ("schema", "models/", "/models.")

        result: dict[str, list[str]] = {}
        sources: list[str] = []

        for path, content in (file_contents or {}).items():
            if any(kw in path.lower() for kw in _SCHEMA_KEYWORDS):
                sources.append(content)

        # Disk fallback when schema files not in agent's file pool
        if not sources and source_dir:
            from pathlib import Path as _Path
            for py in _Path(source_dir).rglob("*.py"):
                py_str = str(py).lower().replace("\\", "/")
                if any(kw in py_str for kw in _SCHEMA_KEYWORDS):
                    try:
                        sources.append(
            ...
            for node in _ast.walk(tree):
                ...
                for child in _ast.iter_child_nodes(node):
                    ...
                    if name.startswith("_"):
                        continue
                    try:
                        type_str = _ast.unparse(child.annotation)
                    except Exception:
                        type_str = "?"
    ...
    @staticmethod
    def _extract_attribute_ref(
        artifacts: list[dict],
        prior_outputs: dict[str, Any],
    ) -> str:
        """AST-extract self._X attributes AND private methods for artifact files.

        Parses classes in the same files as the artifacts, returning:
        - attrs: self._X assignments from __init__ / _init_components
        - methods: all private method names (_foo) defined on the class

        Both sections are needed so P4 can catch:
        1. Wrong self._X attribute access (attribute not in attrs list)
        2. Calls to self._foo() that don't exist (method not in methods list)

        Only covers files that artifacts actually modify — keeps output tiny.
        """
        import ast as _ast
        ...
        # Collect file paths from artifacts
        artifact_paths = {a.get("filename", "") for a in artifacts if a.get("filename")}

        lines: list[str] = []

        for art_path in sorted(artifact_paths):
            # Find source content
            content = (file_contents or {}).get(art_path, "")
            if not content:
                for key, val in (file_contents or {}).items():
                    if key.endswith(art_path) or art_path.endswith(key):
                        content = val
                        break
            if not content and source_dir:
                from pathlib import Path as _Path
                full = _Path(source_dir) / art_path
                if full.is_file():
                    try:
            ...
            for cls_node in _ast.iter_child_nodes(tree):
                ...
                for method in _ast.iter_child_nodes(cls_node):
                    if not isinstance(method, (_ast.FunctionDef, _ast.AsyncFunctionDef)):
                        continue
                    # Collect private method names (excluding dunder)
                    if (method.name.startswith("_")
                            and not method.name.startswith("__")):
                        methods.append(f"{method.name}()")
                    # Collect self._X attribute assignments from init methods
                    if method.name not in ("__init__", "_init_components", "setup"):
                        continue
                    for node in _ast.walk(method):
                        if not isinstance(node, _ast.Assign):
                            continue
                ...
                if attrs or methods:
                    parts = []
                    if attrs:
                        parts.append("attrs: " + ", ".join(attrs[:8]))
                    if methods:
                        parts.append("methods: " + ", ".join(methods[:12]))
                    lines.append(
                        f"  {cls_node.name} ({art_path}): " + "; ".join(parts)
                    )

        return "\n".join(lines)

    async def _ground_artifact_fields(
        self,
        client: Any,
        artifacts: list[dict],
        prior_outputs: dict[str, Any],
    ) -> list[dict]:
        """Post-synthesis grounding: fix wrong field names and internal method calls.

        Runs one cheap LLM call (like P3's contradiction check) asking for
        exact text replacements — not full artifact rewrites. Applied via
        str.replace() so the prompt stays small regardless of artifact size.

        Covers two error types:
        1. Wrong Pydantic model field names (request.X, response.X)
        2. Wrong self._X method/attribute calls on modified classes

        Compact prompt: artifact previews (first 400 chars each) + reference cards.
        Compact output: {"replacements": [{"old": ..., "new": ...}]}.
        Does NOT inject context into synthesis — runs post-synthesis only.
        """
        schema_fields = self._extract_schema_fields(prior_outputs)
        attr_ref = self._extract_attribute_ref(artifacts, prior_outputs)

        if not schema_fields and not attr_ref:
            logger.info(
                f"Stage '{self.name}': field grounding skipped — no reference info"
            )
            return artifacts

        # Compact field card: ClassName: field1, field2, ...
        field_card = "\n".join(
            f"  {cls}: " + ", ".join(f.split(":")[0].strip() for f in fields)
            for cls, fields in sorted(schema_fields.items())
            if fields
        )

        # Artifact previews — only first 400 chars per artifact
        previews = []
        for a in artifacts:
            fname = a.get("filename", "unknown")
            preview = a.get("content", "")[:400]
            previews.append(f"File: {fname}\n{preview}")
        previews_text = "\n---\n".join(previews)

        replacement_schema = '{"replacements": [{"old": "exact text", "new": "corrected text"}]}'

        ref_sections = []
        if field_card:
            ref_sections.append(
                f"## REAL PYDANTIC FIELD NAMES\n{field_card}"
            )
        if attr_ref:
            ref_sections.append(
                f"## REAL self._ ATTRIBUTES AND METHODS (on classes being modified)\n"
                f"(ONLY these attrs/methods exist — any other self._X call is fabricated)\n"
                f"{attr_ref}"
            )
        ref_text = "\n\n".join(ref_sections)

        prompt = (
            "Check these code artifacts for two types of errors:\n"
            "1. Wrong Pydantic model field names (e.g. request.messages "
            "when real field is request.message or request.history)\n"
            "2. Fabricated self._X() method calls — CRITICAL: if the artifact "
            "calls self._foo() but _foo() is NOT listed under 'methods:' in the "
            "reference below, that method does not exist and must be removed "
            "or replaced with a concrete implementation inline.\n\n"
            f"{ref_text}\n\n"
            f"## ARTIFACT CODE PREVIEWS\n{previews_text}\n\n"
            "Output exact text replacements for any errors found.\n"
            f"Return JSON: {replacement_schema}\n"
        ...
        try:
            ...
            if not replacements:
                logger.info(
                    f"Stage '{self.name}': field grounding — no corrections "
                    f"({elapsed:.1f}s)"
                )
                return artifacts

            # Apply replacements via str.replace on full artifact content
            corrected = []
            applied = 0
            for a in artifacts:
                ...
                for r in replacements:
                    ...
                    if old and new and old != new and old in content:
                        ...
                        applied += 1
                corrected.append({**a, "content": content})

            logger.info(
                f"Stage '{self.name}': field grounding applied {applied}/"
                f"{len(replacements)} replacement(s) in {elapsed:.1f}s"
            )
            return corrected

        except Exception as e:
            logger.warning(
                f"Stage '{self.name}': field grounding failed ({e}), "
                "keeping originals"
            )
    ...
    async def execute(
        ...
        try:
            ...
            # P4: post-artifact field grounding — fix wrong Pydantic field names
            # Compact replacement-based pass: does not inject into synthesis prompt
            artifacts_to_ground = design_merged.get("artifacts", [])
            if artifacts_to_ground:
                design_merged["artifacts"] = await self._ground_artifact_fields(
                    client, artifacts_to_ground, prior_outputs,
                )
