...
8. **F10 was NOT the Sonnet score bottleneck**: Run 67 (45.3 avg) had 40% F10 and scored highest. Run 72 (40.3 avg) has 0% executable F10 but scores same as baseline. The `self._xxx` patterns (`_detection_orchestrator()` callable, `_embedder.embed([])`) are the actual score drivers — they appear in 50% of engine.py artifacts across ALL runs and dominate consistency/implementability scores.
