...
| # | Date | Model | Quant | Ctx | Pipeline SHA | Codebase SHA | Pipeline | Decisions | Time | Det-Score | Fab | Field | Syn | Cov | Chars | **Total** | Notes |
| 52a-j | 2026-03-31 | qwen3-coder-next-reap (40B) | Q5_K_S | 65K | `3b8ef270` | fitz-sage main | BASELINE monolithic template (10 runs, det-scorer) | 10-15 | 247-466s | 73.0 | 1.7 | 0.3 | 0.5 | 77% | 1069 | **73.0 det** | Deterministic scorer baseline. Monolithic template extraction. Low fabrication (1.7) but low coverage (77%) — writes less code, fewer chances to fabricate. |
| 53a-j | 2026-03-31 | qwen3-coder-next-reap (40B) | Q5_K_S | 65K | `3b8ef270` | fitz-sage main | PER-ARTIFACT + init preservation + schema injection | 10-15 | 240-466s | 63.5 | 3.5 | 0.2 | 1.1 | 88% | 2640 | **63.5 det** | Per-artifact generation: 1 generate() per file with real source code. Init preservation keeps self._xxx = ClassName(...). Schema field injection. More code (2640 chars avg) but more fabrication (3.5 avg). Trade-off: coverage 77%→88% but fabrications 1.7→3.5. |
| 54a-j | 2026-04-01 | qwen3-coder-next-reap (40B) | Q5_K_S | 65K | `3acf4940` | fitz-sage main | + class interface injection (compressed source bug) | 12-15 | 240-466s | 70.4 | 2.3 | 0.2 | 1.0 | 94% | 2367 | **70.4 det** | Injected available methods on self._xxx component classes. BUT source was pre-compressed (3780 chars), interfaces only populated for small files. Fabrications 3.5→2.3 (-34%), 22% zero-fab runs. Coverage 88%→94%. |
| 55a-g | 2026-04-01 | qwen3-coder-next-reap (40B) | Q5_K_S | 65K | `7e079300` | fitz-sage main | + type-aware repair + test leak filter (no disk fix) | 12-15 | 319-474s | 61.7 | 1.7 | 1.9 | 1.6 | 100% | 1814 | **61.7 det** | Type-aware resolution: reverse map ClassName→attr_name, matches fabricated names against CamelCase type parts. Test leak filter strips self.test_*(). Fabs 2.3→1.7, zero-fab 43%. But field errors spiked (LLM variance), dragging score down. 7 runs. |
...
| 2026-03-31 | synthesis.py | `3b8ef270` | Per-artifact generation: 1 generate() per file with target source | Trade-off: coverage 77%→88%, fabrication 1.7→3.5. More code = more fabrication surface. |
| 2026-03-31 | compressor.py | `3b8ef270` | Init preservation: keep self._xxx = ... in __init__ bodies | Feeds per-artifact with real attr names (was stripped before). |
| 2026-03-31 | synthesis.py | `3b8ef270` | Schema field injection: Pydantic field names from AST | Field errors 3.5→0.2 on per-artifact path. |
| 2026-03-31 | eval_deterministic.py | `3b8ef270` | Deterministic scorer: zero-variance AST-based plan scoring | Replaces Sonnet scorer for reliable A/B comparison. |
| 2026-04-01 | synthesis.py | `3acf4940` | Class interface injection: component methods on self._xxx attrs | Fabrications 3.5→2.3 (-34%). 5216 chars of grounded interface data. |
