...
| 80a-h | 04-07 | + F21 surgical rewrite (F3 leak bug) | 12-16 | — | 7.6 | 5.6 | 4.8 | 5.6 | 4.4 | 7.8 | **35.8 avg** | 6/8 plans. F3 signature leak from surgical rewrite caused 7/37 new build_abstain_message fabrications. Engine.py 7/7 pipeline steps. |
| **81a-e** | **04-07** | **+ F3 leak fix (skip F3 for surgical outputs)** | **12-16** | **—** | **7.4** | **5.6** | **4.6** | **5.0** | **4.4** | **7.0** | **34.0 avg** | **5/5 plans. NEW BASELINE. All plans now attempt engine.py (surgical rewrite). Per-function route decomposition. Old baseline (41) was inflated by 33% of plans having only 1-2 tiny artifacts and 19% skipping engine.py entirely. 6-dimension scorer penalizes plan completeness. New evaluation method needed.** |
