...
_SKIP_NAMES = frozenset(
    {
        ...
        "AttributeError", "NotImplementedError", "StopIteration", "OSError",
        "IOError", "FileNotFoundError", "ImportError", "IndexError",
        "ConnectionError", "TimeoutError", "PermissionError",
        # Stdlib classes commonly constructed directly
        "ThreadPoolExecutor", "ProcessPoolExecutor", "Lock", "Event", "Thread",
        "Queue", "defaultdict", "Counter", "OrderedDict", "deque", "datetime",
        "timedelta", "timezone", "Decimal", "Enum", "IntEnum", "ABC",
        "abstractmethod", "contextmanager", "asynccontextmanager", "wraps",
        "partial", "reduce", "lru_cache", "cached_property",
        # Typing
        "TypeVar", "ParamSpec", "Protocol", "Generic", "ClassVar", "Final",
        "Literal", "Annotated", "NamedTuple", "TypedDict", "cast", "overload",
        "dataclass_transform", "Optional", "Union", "Any", "List", "Dict",
        "Set", "Tuple", "Iterator", "Generator", "AsyncGenerator",
        "AsyncIterator", "Callable", "Type", "Sequence", "Mapping", "Iterable",
        "Awaitable", "Coroutine",
        # Common stdlib functions (may appear as Call().func.id)
        "callable", "reversed", "property", "staticmethod", "classmethod",
        "vars", "dir", "globals", "locals", "exec", "eval", "compile",
        "breakpoint", "print", "len", "range", "enumerate", "zip", "map",
        "filter", "sorted", "isinstance", "issubclass", "hasattr", "getattr",
        "setattr", "any", "all", "min", "max", "sum", "abs", "round", "hash",
        "id", "repr", "open", "iter", "next",
        # Stdlib modules frequently used as attribute roots
        "Path", "logging", "json", "re", "os", "sys", "time", "uuid",
        "asyncio", "functools", "itertools",
        # Dataclasses / pydantic
        "dataclass", "field", "dataclasses", "BaseModel", "Field", "ConfigDict",
        "model_validator", "field_validator", "computed_field",
        # FastAPI / Starlette
        "APIRouter", "Request", "StreamingResponse", "EventSourceResponse",
        "Depends", "HTTPException", "BackgroundTasks", "Response",
        "JSONResponse", "Body", "Header", "Cookie", "Form", "File",
        "UploadFile", "status", "router", "app",
    }
)
...
def _check_node(
    ...
    if (
        ...
        if not lookup.method_exists_anywhere(method_name):
            ...
            violations.append(
                Violation(
                    filename, line, f"self.{method_name}()", "missing_method",
        ...
        if func_name[0].isupper():
            if not lookup.class_exists(func_name):
                if func_name not in artifact_classes:
                    ...
                    violations.append(
                        Violation(
                            filename, line, func_name, "missing_class",
            ...
            if not lookup.function_exists(func_name):
                if not any(func_name.startswith(p) for p in ("_", "pytest")):
                    ...
                    if suggestions:
                        violations.append(
                            Violation(
                                filename, line, f"{func_name}()", "missing_function",
                ...
                if expected_params is not None:
                    ...
                    if actual_args > 0 and expected > 0 and abs(actual_args - expected) > 2:
                        violations.append(
                            Violation(
                                filename, line, f"{func_name}()", "wrong_arity",
        ...
        if type_name:
            ...
            if cls and cls.methods:
                ...
                if (
                    ...
                    violations.append(
                        Violation(
                            filename, line, f"{var_name}.{attr_name}", "wrong_field",
