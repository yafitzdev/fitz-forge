...
from fitz_forge.planning.schemas import ArchitectureOutput, DesignOutput
...
class ArchitectureDesignStage(PipelineStage):
    ...
    def parse_output(self, raw_output: str) -> dict[str, Any]:
        ...
        if recommended not in approach_names and approach_names:
            ...
            if matches:
                ...
                logger.warning(
                    f"recommended '{recommended}' not in approaches, using first: '{approach_names[0]}'"
                )
    ...
    async def _run_verification_agents(
        ...
        contract_sheet = (
            parallel_results[0] if not isinstance(parallel_results[0], Exception) else ""
        )
        data_flow_map = (
            parallel_results[1] if not isinstance(parallel_results[1], Exception) else ""
        )
        pattern_catalog = (
            parallel_results[2] if not isinstance(parallel_results[2], Exception) else ""
        )
        type_boundary_audit = (
            parallel_results[3] if not isinstance(parallel_results[3], Exception) else ""
        )
        ...
        if contract_sheet:
            sections.append(
                f"--- INTERFACE CONTRACTS (verified against source) ---\n{contract_sheet}"
            )
        if data_flow_map:
    ...
    def _build_prompt_parts(
        ...
        if "context" in prior_outputs:
            ...
            if context.get("needed_artifacts"):
                parts.append(
                    f"Expected deliverable files: {', '.join(context['needed_artifacts'])}"
                )
            context_str = "\n".join(parts)
        ...
        if dupes:
            ...
            for d in dupes:
                ...
                dupe_lines.append(
                    "  → CHECK these files before creating new ones. Extend existing code if possible."
                )
            binding += "\n".join(dupe_lines)
